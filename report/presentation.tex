...
\begin{center}
\begin{tikzpicture}
	\onslide<1-4> \node (img1) {\includegraphics[scale=0.18]{../plots/motivation.png}};
	\pause
	\onslide<2> \node (img2) at (img1.south) {\includegraphics[scale=0.18]{../plots/red_arrow.png}};
	\pause
\end{tikzpicture}
\end{center}
\begin{columns}
	\column{0.5\textwidth}
	\onslide<3-4> {2 (similar) datasets with differing size, \# of classes, class balance}		\onslide<3-4>{\includegraphics[width=\textwidth, height=3.5cm]{../plots/class_distro_plot.png}}
	\pause
	\column{0.5\textwidth}
	\onslide<4> {BOHB outperforms SMAC in high dimensional, mixed data}
	\onslide<4> {\includegraphics[width=\textwidth]{../plots/bohb_smac.png}}
\end{columns}
\end{frame}
...
%	\onslide<3->\begin{tabular*}{\textwidth}{|>{\small}c|>{\small}c|>{\small}c|>{\small}c|}
	...
	\column{0.48\textwidth}
	\begin{figure}
		\onslide<2->\includegraphics[height=3.7cm,width=4.5cm]{../plots/plot_performance_histogram.png}
...
\pause
%\vspace{0.2cm}
\onslide<3->{\underline{Issues}: (i) Slow experiments; (ii) Under-represented classes; } 
...
\begin{columns}
	\column{0.5\textwidth}
	\onslide<1->Possible improvements:
	\begin{itemize}
		\onslide<1->\item Fidelity (data subsets)
		\onslide<1->\begin{itemize}
			\item K49: 90.20\% $\rightarrow$ 93.85\%
		\end{itemize} 
		\onslide<2->\item Data Augmentation
		\onslide<2->\begin{itemize}
			\item K49: 94.28\% $\rightarrow$ 93.37\% (with fidelity)
		\end{itemize}
		\onslide<3->\item Better/more parameters for transfer configuration
		\onslide<3->\begin{itemize}
			\item Learning rate schedule
			\item Optimizer
		\end{itemize}
		\onslide<4->\item EPMs to try reduce parameter blow ups and runtimes
		\onslide<5->\item NAS cell search
		\onslide<6->\item Padding to go deeper (exploit sparsity)
	\end{itemize}
	\hrule
	\vspace{0.2cm}
	\onslide<7->{\underline{Final Numbers}:}
	\vspace{0.15cm}
	\onslide<7->\begin{tabular}{|c|c|c|}
		%	\onslide<3->\begin{tabular*}{\textwidth}{|>{\small}c|>{\small}c|>{\small}c|>{\small}c|}
		\hline
...
%		\multicolumn{1}{|p{1cm}|}{\centering Best \\ Test} \\
		...
		\hline
	\end{tabular}
	\onslide<7->{\underline{KMNIST}: \textit{from transferring best K49 model}}\\
	\onslide<7->{\underline{K49}: \textit{from transferring incumbent config on KMNIST and then BOHB on K49}}\\


	\column{0.5\textwidth}
%	\onslide<1>{\includegraphics[width=\textwidth]{../plots/plot_correlation_across_budgets.png}}
	\begin{onlyenv}<1-7>
	\begin{center}
		\includegraphics<1>[height=2cm,width=2cm]{../plots/plot_correlation_across_budgets.png}
		\includegraphics<7>[height=4cm,width=\textwidth]{../plots/kmnist_confusion_matrix_default.png}
		\includegraphics<7>[height=4cm,width=\textwidth]{../plots/confusion_matrix.png}
	\end{center}
	\end{onlyenv}
\end{columns}
\end{frame}
